...
/*  Time: O(n * m)  n is the number of pairs, m is the number of queries   Space: O(n)
    equations: 
    [
        ["a", "b"]
        ["b", "c"]
    ...
    Use a new class to restore the the divide pair,
    a map to mapping the Key
    
    Map<String, List<Cell>>
    then it becomes a graph problem, from the start to the target, keep track of the mapping relation of each node
    if a/b = 2, then if we want to go from a to b, record the 2 in our list, if wanna from b to a, record -2 in out list
    
    then if we DFS the target value and got it, we calculate the records
*/